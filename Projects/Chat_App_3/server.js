...
io.sockets.on('connection', function(socket){
	...
	socket.on('new user', function(data, callback){
		...
		if(data in restaurants){
			...
			restaurants[socket.nickname] = socket;
			updateNickname();
		}
	});
		...
		if(msg.substr(0,3) === '/w '){
			...
			if(ind !== -1){
				...
				if(name in restaurants){
					restaurants[name].emit('whisper', { msg : msg, nickname : socket.nickname });
				} else {
	...
	socket.on('disconnect', function(data){
		...
		delete restaurants[socket.nickname];
		updateNickname();
	});

	function updateNickname(){
		//io.sockets.emit('username', nicknames);
		io.sockets.emit('username', Object.keys(restaurants));
	}
});
